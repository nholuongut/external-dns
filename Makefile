...
BUILD_FLAGS   ?= -v
LDFLAGS       ?= -X github.com/nholuongut/external-dns/pkg/apis/externaldns.Version=$(VERSION) -w -s
ARCH          ?= amd64
SHELL          = /bin/bash
...
build.push/multiarch: ko
    ...
    ko build --tags ${VERSION} --bare --sbom ${IMG_SBOM} \
      --image-label org.opencontainers.image.source="https://github.com/nholuongut/external-dns/external-dns" \
